...
# Speechformer by https://github.com/HappyColor/
...
# Speech-MHA (Speechformer 2) by https://github.com/HappyColor/SpeechFormer2
...
class transformer_encoder(nn.Module):
    ...
    1. Transformer Encoder: Implements a Transformer-based architecture tailored for speech tasks, utilizing self-attention mechanisms.
    2. Subsampling Frontend: Uses a 2D Convolutional Subsampling module to reduce time resolution and extract local spectral features.
    3. Positional Encoding: Adds positional information to input embeddings, enabling the self-attention mechanism to recognize sequence order.
    4. Multi-Head Attention: Stacks multiple Encoder Layers, each containing Multi-Head Self-Attention and Position-wise Feed-Forward networks.
    5. Global Pooling: Aggregates the sequence of encoded vectors into a single representation using Global Max Pooling, followed by a post-processing MLP block.
    '''
    def __init__(self, features, classes, kernel_size=2, d_model=252, dropout_rate=0.3, num_blocks=12, n_heads=12, linear_units=2048, normalize_before=True, concat_after=False, stochastic_depth_rate=0.3, add_pos_enc=True):
        ...
        self.adaptavgpool = nn.AdaptiveAvgPool1d(8)
        ...
        speechencoder_selfattn_layer = Speech_MultiHeadedAttention
        ...
        self.attention = torch.nn.Sequential(
           torch.nn.Linear(d_model, int(d_model)),
           torch.nn.LayerNorm(int(d_model)),
           torch.nn.Tanh(),
           torch.nn.MaxPool1d(2,2),
        )
        self.fc=nn.Linear(int(d_model/2), classes)
    ...
    def forward(self, x, src_mask):   
        ...
        x = x.float()
        x, x_mask = self.subsampling(x, src_mask)

        if self.pos_enc_bool==True:
            x=self.pos_enc(x)

        x, _ = self.encoders(x, x_mask)

        if self.normalize_before==True:
            x=self.after_norm(x)
        
        # x = self.maxpool(x)
        x, _ = torch.max(x, dim=1) # im vgl. zu torch.mean und flatten hat torch.max die besten Ergebnisse geliefert
