...
class DataLoader():
    ...
    def __getitem__(self, idx):
        ...
        self.ndarrayWav=self.dataset[i][:]
        if self.transform:
            self.ndarrayWav=self.transform(self.ndarrayWav)

        self.label=list(self.dataset[i].attrs.items())[0][1]
        return self.ndarrayWav, self.label, self.name
    
    def __len__(self):
        return self.dataset_len

# Padding from ESPnet https://github.com/espnet/espnet/tree/master
...
class Trainer():
    ...
    def train(self, n_epochs, train_accu=None, eval_accu=None):
        ...
        for epoch in range(n_epochs):
            ...
            pred_list = []
            target_list = []
            for i, (mel_train, label_train, name, src_mask) in enumerate(self.training_dataset):
                data_train=mel_train
                self.optimizer.zero_grad()
                label_train = label_train.data.squeeze(1)
                outputs_train=self.model(data_train, src_mask)
                label_train_one = F.one_hot(label_train, num_classes=7).float()
            ...
            self.scheduler.step()
            print(f'Decreasing lr: {self.optimizer.param_groups[0]["lr"]}')
            t2 = time.time()
            epoch_time=t2-t1
            ...
            accuracy_valid, _ = self.eval(epoch, self.validation_dataset)
            ...
            eval_accuracy.append(accuracy_valid)
            
        return train_accuracy, eval_accuracy, self.best_model_state_dict
    ...
    def eval(self, epoch, dataset_eval):
        ...
        target_list = []
        with torch.no_grad():
            for i, (mel_valid, label_valid, _, src_mask) in enumerate(self.validation_dataset):
            ...
            return accuracy_valid, best_model
    ...
    def testing(self):
        ...
        idx=0
        with torch.no_grad():
            for i, (mel_test, label_test, _, src_mask) in enumerate(self.testing_dataset):
                data_test=mel_test
            ...
            label_names=["disgust", "angry", "fear", "happy", "neutral", "pleasant surprise", "sad"]
            ...
            plt.title('TESS Confusion Matrix', fontsize=30, fontweight='bold', pad=20)
    ...
    def execute(self):
        ...
        training_dataset_path = os.path.join(base_path, "Dataset/train_TESS_mel_mfcc.h5")
        testing_dataset_path = os.path.join(base_path, "Dataset/test_TESS_mel_mfcc.h5")
        valid_dataset_path = os.path.join(base_path, "Dataset/valid_TESS_mel_mfcc.h5")
        print(os.getcwd())
        ...
        # print(len(self.testing_dataset.__getitem__(0)[0]))
        ...
        accuracy_train_total, accuracy_valid_total, best_model_state_dict = self.train(self.n_epochs)
        accuracy_test_total, cm = self.testing()
        ...
        with open(f"{file_path}.txt", "w+") as file_spec:
            ...
            file_spec.write(f"Test Accuracy: {accuracy_test_total} \n\n")
            file_spec.close()
